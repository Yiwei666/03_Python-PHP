...
import socks
import socket
import concurrent.futures  # 这里增加对 concurrent.futures 的导入
...
def scp_transfer_file(file_info, remote_path, remote_host, remote_port, username, password):
    ...
    try:
        ...
        # 检查远程文件是否存在，并且文件大小是否相同
        try:
            remote_file_info = sftp.stat(remote_file)
            local_file_size = os.path.getsize(local_file)
            
            if remote_file_info.st_size == local_file_size:
                print(f"File {local_file} already exists and has the same size. Skipping.")
                return  # 如果文件存在且大小相同，则跳过上传
