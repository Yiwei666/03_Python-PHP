...
<head>
    ...
    <style>
        ...
        .video-row .video {
            flex-basis: calc(100% / <?php echo $videosPerRow ?? 2; ?> - var(--video-column-gap) * 2);
...
<body>
    ...
    $videos_on_disk = glob($videoPath . '*.mp4');

    // --- 代码修改开始 ---

    // 1. 一次性从数据库获取所有视频的点赞数
    $all_videos_from_db = [];
    if ($result = $mysqli->query("SELECT video_name, likes FROM tk_videos")) {
        while ($row = $result->fetch_assoc()) {
            $all_videos_from_db[$row['video_name']] = (int)$row['likes'];
        }
        $result->free();
    }

    // 2. 整理视频列表，并同步数据库（将新视频加入数据库）
    $video_details = [];
    foreach ($videos_on_disk as $video_path) {
        $videoName = basename($video_path);
        if (array_key_exists($videoName, $all_videos_from_db)) {
            // 视频已存在于数据库
            $video_details[] = ['name' => $videoName, 'likes' => $all_videos_from_db[$videoName]];
        } else {
            // 发现新视频，插入数据库，点赞数默认为0
            ...
            if ($insert_stmt) {
                ...
                $insert_stmt->close();
                // 添加到当前列表用于显示
                $video_details[] = ['name' => $videoName, 'likes' => 0];
            }
        }
    }

    // 3. 根据 likes 字段对视频列表进行降序排序
    usort($video_details, function($a, $b) {
        return $b['likes'] <=> $a['likes']; // PHP 7+ 太空船操作符
    });

    // 4. 基于排序后的视频列表进行分页
    $videos = $video_details; // 使用已排序的数组
