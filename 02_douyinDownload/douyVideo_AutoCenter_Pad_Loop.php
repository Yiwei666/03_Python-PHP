...
<body>
    ...
    <script>
        ...
        function toggleLoop(button) {
            var video = button.nextElementSibling.nextElementSibling; // 获取按钮相邻的视频播放器
            video.loop = !video.loop; // 切换LoopPlay状态

            var cancelLoopButton = button.nextElementSibling;
            if (video.loop) {
                button.textContent = 'AutoPlay';
                cancelLoopButton.style.display = 'inline-block'; // 显示AutoPlay按钮
            } else {
                button.textContent = 'LoopPlay';
                cancelLoopButton.style.display = 'none'; // 隐藏AutoPlay按钮
            }
        }

        function cancelLoop(button) {
            var video = button.nextElementSibling; // 获取按钮相邻的视频播放器
            video.loop = false; // AutoPlay
            button.style.display = 'none'; // 隐藏AutoPlay按钮
        }
    </script>
