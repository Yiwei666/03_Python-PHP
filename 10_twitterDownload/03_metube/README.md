...
docker logs --tail=300 metube          # 查看docker中的 metube 报错日志

docker update --restart unless-stopped 容器名或容器ID    # 服务器重启、Docker 重启、容器异常退出后会自动拉起；

docker-compose up -d                   # 运行命令,让容器在后台运行，
                                       # 命令会根据当前目录下的 docker-compose.yml 文件的定义，启动指定的服务。
                                       # -d 标志表示以守护进程（后台）模式运行服务，即服务将在后台继续运行而不会占用当前终端。

docker port <容器名称或容器ID>          # 查看容器的端口映射关系

docker inspect <容器名称或容器ID>       # 查看容器完整信息
...
当用户访问 `https://你的域名/metube/` 时，由 Nginx 把请求转发到服务器本机的 `http://127.0.0.1:8081/`，也就是转发给 MeTube 容器。
...
访问 `https://你的域名/metube/` 时，先要求输入用户名和密码；认证通过后，Nginx 才会把请求转发到本机 `127.0.0.1:8081` 上的 MeTube 服务。
