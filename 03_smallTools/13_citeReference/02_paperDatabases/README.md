...
2. 打印出 `papers` 表中的所有列名，并且要有序号。询问用户是否需要手动修改该doi号所在行的某一列的值，如果需要请输入该列名对应的序号，不需要的话则输入q结束程序运行，输入除了q和列序号之外的其他值均为非法输入，提示用户重新输入。注意：不允许用户修改 `paperID` 列的值，即输入该列对应的序号也是非法的。因为该值被数据库中的多个表公用，是论文在数据库中的唯一标识。
