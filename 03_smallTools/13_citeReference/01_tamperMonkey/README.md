...
2. 注意：CrossRef API 返回的元数据涵盖了学术文章的全面信息，包括标题、作者（含完整姓名和 ORCID 标识符）、期刊名、卷号、期号、页码、文章编号、DOI（数字对象标识符）及其标准链接（URL）。此外，还提供出版商名称、出版日期（在线或纸质）、ISSN/ISBN 编号、文章摘要、语言、学科分类、参考文献列表、文章类型（如期刊文章或会议论文）、基金资助信息（资助方名称和编号）、版权许可信息（含开放获取状态和许可证链接）以及文章关键字等。这些数据可进一步扩展到引用次数、文章标识符、全文链接及多语言标题等，确保对文献的详细描述与学术用途的支持。
