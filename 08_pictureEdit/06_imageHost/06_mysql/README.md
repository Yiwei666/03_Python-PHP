...
上述脚本从数据库中获取所有 `image_exists = 1` 的图片；若 URL 中传入分类 ID（cat 参数），则只在该分类下进行图片筛选；根据排序类型对图片排序；确定当前图片在排序后数组中的位置，并获取其前后图片用于导航。 左右箭头用于查看上一张或下一张图片，支持分类内导航。 

现在我需要添加一个有关导航的简单功能，具体分析和要求如下：

1. 当网络带宽拥挤或者图片较大时，用户使用左右箭头用于查看上一张或下一张图片时（尤其是下一张图片，上一张图片可能由于刚浏览过会被浏览器缓存），图片完全加载可能较为耗时，用户浏览体验不好。

2. 我想到的一个方法是预加载，例如假如当前图片在相应排序中的索引是i，当当前图片加载完毕后（用户在浏览时），可以预加载 i-1，i-2，以及 i+1 和 i+2 这4张图片（如果用户向后浏览，i-1，i-2图片可能已被加载过了），当这4张图片被预加载缓存后，用户浏览完当前图片后，使用左右箭头切换到下一张的加载时间会大大降低，浏览体验会提升。

3. 当用户由当前图片 i 切换到 i+1 时，浏览器需要预加载 i+3，（其中i+2图片在用户浏览图片i时，已被加载），以此顺推。

4. 假如用户从后往前切换图片，即用户 当前图片 i 切换到 i-1 时，浏览器需要预加载 i-3，（其中i-2图片在用户浏览图片i时，已被加载），以此顺推。

5. 需要保证当前图片向前和向后的各2张图片都要提前被预加载。
...
6. 始终预加载当前索引前后两张，切换到新索引后也会在新页面自动预加载它的前后两张。
...
访问 `08_picDisplay_mysql_orderExistTab.php` 这个页面时，图片分为若干页，每页显示固定数量的图片，点击 🔁 图标，会跳转`08_image_leftRight_navigation.php`显示相应的图片，图片的右上角会显示当前图片所属的分类。现在我的需求如下：
1. 在 `08_picDisplay_mysql_orderExistTab.php` 这个页面中每张图片的右上角同样显示当前图片所属分类，方便用户快速查看。
2. 相关实现可以参考/复用已有的显示图片分类的方法。
...
同样：
1. 对于 `08_picDisplay_mysql_galleryExistTab.php` 显示的页面，同样需要每张图片的右上角同样显示当前图片所属分类，方便用户快速查看。
2. 相关实现可以参考上述 `08_picDisplay_mysql_orderExistTab.php` 脚本中刚才新增的有关该功能的代码。
...
访问 `08_picDisplay_mysql_galleryExistTab.php` 这个页面中某个分类下的某一页，例如`https://example.com/08_picDisplay_mysql_galleryExistTab.php?page=26&category=28`，如果想要将这个页面中多张图片新增到另外一个或多个相同分类下，需要逐一点击 每张图片下的 分类管理按钮 🎨，然后勾选分类，再保存，我认为这样效率比较低。我想到了另外一种方法，如下：
1. 在 `08_picDisplay_mysql_galleryExistTab.php` 这个页面中左侧`分类`按钮下面新增一个固定文字按钮`批量写入`，在页面中每张图片卡片左上角放一个复选框，用户可以在这个页面中同时勾选多个图片，然后点击`批量写入`按钮，在弹出的窗口中，只展示分类列表，勾选相应分类，可以将这些图片批量保存到相应分类下。这个弹出的窗口样式可以类似于点击 🎨 按钮所弹出的窗口。保存时执行给这些图片添加所选分类，不删除它们已有分类。
2. 由于所勾选的这些图片可能本身就所属于不同的分类，因此点击 `批量写入` 按钮所弹出的窗口中，没有办法展示所勾选的图片已经所属的分类，这点区别于 🎨 按钮所弹出的窗口能够通过复选框勾选的方式展示相应图片所属的分类。
3. 需要注意用户所勾选的多张图片，然后统一保存到某个或者某几个分类时，有可能其中有些图片已经所属到其中一个或者多个分类下了，因此最好要考虑到这种情况，从优雅实现来说，最好不要重复写入某个图片和分类的映射关系，必须要考虑到用户由于主观或者失误出现重复写入分类的情况。
4. 原来的每张图 🎨 完全保留，继续用于单张图片完整修改分类。新增的 `批量写入`按钮功能不要影响到 原来每张图片下的 🎨 按钮功能，以及脚本原有的其他功能。
5. 不跨页保留勾选状态。翻页后选择清空。刷新页面后选择清空。
6. 只新增缺失的分类关系，不删除任何已有分类，不重复插入已有关系。例如图片 A 已经属于分类 28 和 30，批量选择分类 30 和 35，那么最终只新增 `A -> 35，A -> 30` 不重复写。
7. 批量窗口里的复选框应默认全部不选，只表示“本次要新增写入的分类”。已勾选的复选框最好有美观且醒目的颜色填充，方便用户快速辨识哪些勾选了。
8. 保存成功后提示 `写入了多少条新关系、跳过了多少条已存在关系`，并且刷新页面，这样页面中会在每张图片上展示最新的所有分类所属。
9. 左侧 分类 按钮下面新增固定按钮 批量写入。可以再加一个小状态，例如按钮文字变成 批量写入(6)，表示当前已选 6 张。没选图片时点击提示“请先选择图片”。不需要“全选本页 / 清空选择”。
10. 可以在 `08_image_web_category.php` 里新增一个专门动作，逻辑是“批量追加关系”。以便和原有单图 🎨 完全隔离，降低风险。

结合你的建议，我重新修改了prompt，以便减少误判，现在是否还有其他建议和需要编码前沟通的吗？如果没有，请开始编码。
...
3. 调用`08_image_web_category.php`，支持对选择的多张图片写入到一个或者多个相同的分类
...
## 7. `08_web_image_table_statics.php` 数据报表
...
假设数据库中各分类下的图片总数为x（同一张图片可能属于超过1个分类，1张图片属不同分类需重复计数），数据库中图片的总数为y（不考虑不同分类重复计数），数据库中不属于任何分类的图片数量为z，数据库中具有分类的图片数量为w（约束关系应当满足`y=z+w`），现在需要显示如下表格：
- 第1列显示图片分类名，第2列显示对应的`kindID`，
- 第3列显示该分类下的图片总数a，第4列显示该分类下的图片占比 （`a/x*100%`，保留小数点后1位）
- 第5列显示该分类下 `image_exists = 1`的图片数量b，第6列显示该分类下 `image_exists = 0`的图片数量c，第7列显示该分类下 `image_exists = 1`的图片占比d（`d=b/a*100%`，保留小数点后1位）
- 第8列显示该分类下 `likes - dislikes` 在[30, +∞）区间的图片数量e，其中该区间中  `image_exists = 1` 的图片数量为f，第9列显示该区间图片占比 `e/a*100%`，第10列显示该区间中`image_exists = 1` 的图片占比 `f/e*100%`，均保留一位小数
...
对于 `08_web_image_categorylink.php` 脚本，除了 `Twitter`、`Gallery`、`Order`、`Archive`  这4个外，再新增一个 `Sotwe` 文字标签，功能与其余4个标签类似，主要区别在于跳转的网站不同。该文字标签的具体要求如下：

- 点击`Sotwe`页面对应的分类名时，会跳转打开 `https://www.sotwe.com/$kindID`，注意，这个链接中只有`$kindID`是变量值，注意表中少数的 `kindID` 列值可能为空。
