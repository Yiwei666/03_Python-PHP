...
当网络不好，或者网络延迟较高的时候，在`08_web_crossRef_query.php` 网页中，对于一篇没有写入到数据库的新论文，无论是 title 检索模式还是 doi 检索模式结果中，点击 `标签` 和 `分类`按钮将论文的元信息写入到数据库时，会有极少数的情况会把论文信息重复写入到数据库，这种情况往往是在点击 `标签` 和 `分类`按钮加载窗口反应比较慢，或者勾选分类后点击保存延迟较高时出现。具体表现为同一条论文会被重复写入，两条连续的paperID都是同一条论文信息。实际上，论文在数据库写入时专门设计了避免重复写入。我认为可能与网络延迟有关，也有可能是服务器负载比较高的时候，数据库查询出现滞后，我不太确定具体原因。请读取项目文件夹中的所有代码，找到出现这种情况的原因，并通过最少且优雅的代码改动来解决这个重复写入的问题。

注意，使用尽量少的代码修改实现上述需求，以便减少我review代码的工作量。输出修改后的代码。

不要修改与这个需求无关的代码部分，包括注释、换行这些，任何无关的都不要改，哪怕加一个空格。使用最小的代码改动实现上述需求。输出修改后的完整代码，以便我能进行复制粘贴和审查


- 分析和对策

`08_tm_add_paper.php` 原来是“先查 DOI 是否存在，再插入”，在网络慢或重复点击导致两个请求并发时，两个请求可能都先查到“不存在”，随后各自插入，形成重复。已用最小改动在该 DOI 写入前加 MySQL 命名锁，同一个 DOI 的新增请求会串行执行；第二个请求拿到锁后会重新查到已有论文并返回已有 `paperID`。
...
08_webAccessPaper.php
    ...
    # 2. 后端API调用
        ...
        08_web_user_select_tmp.php              # 后端api接口，支持将网页上 `插入临表 清除临表 复制临表` 三个按钮对应的数据操作写入到服务器数据库的 `select_paper` 表中
            08_api_auth.php
            08_db_config.php
        08_tm_get_gdfile_id.php                 # 后端api接口，接收前端发送的doi或paperID参数（点击预览按钮后），查询 gdfile 表格，返回pdf论文的 fileID 参数
            08_api_auth.php
            08_db_config.php
            08_category_operations.php
```
...
运行上述 `08_webAccessPaper.php` 代码，网页右侧显示左侧相应分类标签下的所有论文（包含标题、作者、期刊名等信息），每篇论文下方还会显示 `"标签 评分 下载 复制DOI 复制编码DOI 复制元信息"` 等按钮；再下面的一行是 "分类标签："，包含该论文所属的分类；最下面一行是论文的 rating 显示等（包括星星填充显示和具体的 rating 值）和被引数显示。现在我有一个新的需求，如下：

1. 在 “复制DOI” 、“复制编码DOI”、"复制元信息" 等按钮所在行中 “复制元信息” 按钮后面适当间距新增一个按钮 "预览"，点击该按钮，能够在一个新的标签页中访问网址 `https://drive.google.com/file/d/fileID/view`，其中 `fileID` 是变量，可根据相应论文的 `paperID`或者`doi`来确定，这些信息可从`paper_db`数据库中的 `gdfile` 表中查询到。

2. 编写后端接口 `08_tm_get_gdfile_id.php`，点击 "预览" 按钮时调用该接口访问数据库返回 `fileID` 值。

2. "预览" 按钮的字体大小、字号、颜色等样式均与 “复制DOI” 、“复制编码DOI”、"复制元信息" 等按钮样式保持一致即可。注意：点击 "预览" 按钮后，其字体颜色也需要变成 `#c58af9`，实现可以参考 “复制DOI” 、“复制编码DOI” 等按钮，方便用户分辨是否点击过按钮。

上述需求的实现可能涉及到对`08_webAccessPaper.php`代码的修改。对于上述相关代码修改，尽量通过增加/删减/调整少量代码行来实现。其余部分代码行不要变动，哪怕是增加空格或者修改注释都不行，确保所有的代码修改均与上述需求的实现有关，因为无关的改动会增加我review代码的工作量。输出修改后的完整`08_webAccessPaper.php`。


💡 **20.2 新增思路**

运行上述 `08_webAccessPaper.php` 代码，网页右侧显示左侧相应分类标签下的所有论文（包含标题、作者、期刊名等信息），每篇论文下方还会显示 `"标签 评分 下载 复制DOI 复制编码DOI 复制元信息"` 等按钮；再下面的一行是 `"分类标签："`，包含该论文所属的分类；最下面一行是论文的 rating 显示等（包括星星填充显示和具体的 rating 值）、被引数显示、已选数 和 序号。现在我有一个新的需求，如下：

1. 在序号显示的信息后面适当距离处新增显示 "paperID："，显示的字体大小、颜色、样式等与 "被引数显示、已选数 和 序号" 一致皆可。
...
根据你的建议，现在需要修改 `08_webAccessPaper.php` 代码，新增一个 “复制已选”按钮，要求如下：
1. 
样式和间距需要与`工具 全部下载 全部删除 插入临表 清除临表 复制临表 复制类表`这些按钮一致。点击“复制已选”按钮，对当前勾选项对应论文的 paperID、doi、title以及对doi号进行base32编码后的字符串进行显示和复制，功能有点类似`复制临表`和`复制类表`按钮，但是针对的论文对象有所区别。正如你的建议，可以不经过服务器，体验更轻。

2. 请使用尽可能少的代码改动来实现上述功能，同时兼顾一定的性能。只需要告诉我怎么修改代码即可。输出显示修改后的完整 `08_webAccessPaper.php` 代码，尽量不要改动其他与本功能实现无关的代码，哪怕是注释，以便减少我审查代码的工作量。

3. 请基于项目文件中的 `08_webAccessPaper.php` 进行修改，该代码已经更新过 `复制类表`的功能。

**新增功能特性概括**：新增“复制已选”按钮，可直接复制当前勾选论文的 paperID、doi、title 和 Base32 编码后的 encodedDOI 为 JSON 数据。
该功能复用页面已有的论文数据和勾选状态，不经过服务器请求，操作更轻量。
...
`08_web_crossRef_query.php` 这个脚本实现的是一个网页版 CrossRef 论文检索、结果展示、DOI 编码、论文入库、PDF 预览和分类管理页面。
...
2. **展示 CrossRef 返回的论文元数据**
   ...
   无论是 `Title` 模式还是 `DOI` 模式，页面输出、数据库写入、复制 DOI、复制 Base32、预览判断等后续操作所使用的 DOI，均来自 CrossRef 返回结果中的 `DOI` 字段，而不是直接使用用户输入的原始字符串。

3. **生成并复制 DOI 的 Base32 编码**

   页面中每条结果下方有 `复制 Base32` 按钮，使用脚本内的 `toBase32()` 函数把 CrossRef 返回的 DOI 转成 RFC 4648 Base32 编码，方便用作 PDF 文件名。

   当前 Base32 编码直接基于 CrossRef 返回的 DOI 字符串执行，脚本没有额外把 DOI 统一转成小写。因此，如果 DOI 大小写不同，Base32 编码结果也会不同。

4. **复制 DOI 和前往论文页面**

   每条结果下方新增 `复制 DOI` 按钮，可复制当前结果中的 DOI。

   每条结果下方还有 `前往论文` 按钮，点击后会在新标签页打开 `https://doi.org/{doi}`。
...
5. **检测 DOI 是否已存在于数据库**
   ...
   然后通过 `getAllDois($mysqli)` 获取数据库已有 DOI，并注入到前端变量 `ALL_DOIS`。

   每条 CrossRef 检索结果渲染时，脚本会用当前结果的 DOI 与 `ALL_DOIS` 中的数据库 DOI 进行比较。比较时会把双方都转换为小写，因此 DOI 大小写差异通常不会影响前端“是否已存在”的识别。

   目前判断逻辑使用的是“包含式匹配”：只要当前 DOI 字符串包含数据库中的某个 DOI，就会被认为已存在。因此，如果支持材料 DOI 包含正文 DOI 作为前缀，可能被误判为数据库已存在。

6. **已存在 DOI 的结果高亮显示**

   如果某条结果被判断为 DOI 已存在于数据库，则整条检索结果会添加 `doi-exists` 样式，文字显示为红色 `#c62828`。

   当前版本不再显示“！！！已存在doi号”或“不存在doi，待写入”等提示文字，而是通过整条结果的字体颜色区分是否已存在。

7. **已存在 DOI 时支持 PDF 预览**

   如果某条结果的 DOI 被判断为已存在于数据库，则该结果下方会额外显示 `预览` 按钮。

   点击 `预览` 后，脚本调用：

   - `08_tm_get_gdfile_id.php`

   根据 DOI 查询 Google Drive 文件 ID，并在新标签页打开对应的 PDF 预览链接：

   - `https://drive.google.com/file/d/{fileID}/view`

8. **按钮点击后改变颜色**

   结果区域中的按钮被点击后，会添加 `clicked-btn` 样式，使按钮文字颜色变为 `#fff59d`。

   该逻辑基于 `.item-buttons button` 统一监听，因此适用于 `复制 Base32`、`标签`、`前往论文`、`复制 DOI`、`预览` 等按钮，也兼容后续新增在结果按钮区中的其他按钮。

9. **点击“标签”按钮时写入论文并管理分类**
...
> 在 title 检索模式和 DOI 检索模式下，项目用于页面输出、数据库写入、复制 DOI、复制 Base32 和预览判断的 DOI，都是来自 CrossRef 查询返回结果中的 DOI 字段，而不是直接使用用户输入的 DOI。因此，用户在 DOI 模式下输入大写、小写或混合大小写的 DOI 时，这个输入字符串本身不会直接写入数据库，而是先经过 CrossRef 查询，再使用 CrossRef 返回的 DOI。这个机制在一定程度上实现了 DOI 来源的统一和标准化。
> 
> 目前项目写入数据库时，并没有主动把 DOI 转换为小写，而是保留 CrossRef 返回 DOI 的原始形式。title 模式下写入的是所选检索结果的 CrossRef DOI；DOI 模式下写入的是 CrossRef 单条返回结果中的 DOI。因此，如果 CrossRef 对同一 DOI 始终返回一致格式，那么数据库中的 DOI 也会比较统一；但项目本身并没有额外保证 DOI 一定小写。
目前页面判断 DOI 是否已存在于数据库，是先在服务器端读取数据库中所有 DOI，注入到前端，然后在前端将“当前 CrossRef 返回 DOI”和“数据库已有 DOI”转为小写后进行比对。因此，大小写差异一般不会影响前端页面对 DOI 是否已存在的识别。
> 
> 不过，当前判断逻辑使用的是“包含式匹配”，不是完整相等匹配。也就是说，如果数据库中已有正文 DOI，而 CrossRef 返回的是某个支撑材料 DOI，且支撑材料 DOI 字符串包含正文 DOI，那么当前项目可能会误判为该支撑材料 DOI 已存在于数据库。这个问题与大小写无关，主要来自包含式匹配。
> 
> Base32 编码方面，项目目前直接对 CrossRef 返回的 DOI 字符串进行编码，没有先统一转小写。Base32 编码是逐字符编码，因此大小写会影响编码结果。也就是说，如果同一 DOI 以不同大小写形式进入 Base32 编码，可能生成不同的 Base32 字符串和 PDF 文件名。
> 
> 所以，目前 DOI 大小写对项目功能的影响是：对前端“是否已存在”的判断影响较小，因为比对时转成了小写；对数据库写入是否重复，取决于数据库字段的 collation 以及后端查询行为；对 Base32 编码和 PDF 文件命名则有明确影响，因为大小写不同会生成不同编码。总体上，项目通过 CrossRef 返回 DOI 实现了一定程度的统一，但尚未在项目内部彻底消除 DOI 大小写对 Base32 文件名一致性的影响。
...
上述 `08_web_crossRef_query.php` 显示的页面支持两种查询方式：基于 `title` 和 `doi` 查询，通过 `crossref api` 返回的数据提取相应论文的部分元信息：doi、出版年、期刊、卷号、期号、页码等，点击`标签`按钮后将相应的信息写入到数据 `paper_db` 数据库的 `papers` 表中。现在我有一个新的需求，如下：

1. 用户基于 doi 进行api查询时，查询的 doi 号可能之前已经存在于数据库中了，但是检索结果的页面中似乎是没有相关提示。用户需要点击 `标签` 按钮，来查看该论文的分类标签是否有勾选，以此来判断该论文信息是否已经存在于数据库，不够直观和方便。现在我需要在检索的结果中，直接显示数据库中是否已存在该论文doi。

2. 现在我的需求是，用户基于doi进行api请求的时候，同步检查该doi是否存在于数据库，并在检索结果的页面中给出提示。用户基于doi进行api请求的时候，同步获取服务器数据库中的所有doi，然后将用户输入的doi字符串与数据库中的每一个doi号进行对比（api请求响应通常需要1-3秒的时间，这个等待时间可以用来查询比对数据库中的doi）。

3. 获取数据库中的所有 doi 可以通过在 `08_category_operations.php` 模块中新增一个查询函数来实现。然后在 `08_web_crossRef_query.php` 中直接调用该模块的函数（注意不是通过编写后端api来获取，因为相应和请求需要时间，不够快捷）来获取所有doi，并与用户查询的 doi 字符串进行比对。

4. 若数据库中存在doi号是用户检索的doi的一部分或者与用户检索的doi完全相同，则认为该论文信息已经存在于数据库中（api请求对用户输入的doi字符串格式要求较宽泛，因此判断数据库的doi是否是用户检索doi的部分片段即可，因为用户检索的doi可能包含 `https://`、`doi:`等额外的字符串，但其一定包含最核心的doi号）。

5. 如果数据库中存在相应 doi，则在 `复制 Base32` 按钮下方用红色的24px加粗字体显示 `！！！已存在doi号`；如果不存在，则用绿色的24px加粗字体显示"不存在doi，待写入"。显示的提示与`复制 Base32` 按钮在竖直方向上保持适当间距即可。用户基于 title 字符串进行检索的情况暂不考虑，其他功能不变。`08_web_crossRef_query.php` 页面显示的 `标签` 按钮和 `复制 Base32` 按钮需要位于同一水平高度，两个按钮的大小、样式、相对位置和初始代码一样即可，不需要进行调整。

6. 目前代码已经实现：点击 `标签` 按钮会弹出一个小窗，并显示数据库中所有的分类标签，每个分类标签前都有一个勾选框，通过对号勾选的方式显示当前doi所属的分类标签。用户可以通过勾选或者取消勾选框来实现对论文所属分类的调整。这部分功能不需要改变，只需要在显示的样式上做一些微调。现在我的需求是，对于已经勾选的分类标签，其字体颜色调整为 `#1a0dab`，没有勾选的分类标签其字体颜色不变，方便用户快速分辨和定位哪些分类标签已被勾选。被 勾选的分类标签通常分为两类，一类是数据库中该论文原有所属的分类，弹窗时被加载显示出来的，还有一类是用户新勾选的，这两类都需要显示 `#1a0dab`；当用户在弹窗中取消勾选后，字体的颜色也要相应变为默认的颜色。

7. 上述需求的实现可能涉及到对`08_category_operations.php`和`08_web_crossRef_query.php`代码的修改。对于上述相关代码修改，尽量通过增加/调整少量代码行来实现。其余部分代码行不要变动，哪怕是增加空格或者修改注释都不行，确保所有的代码修改均与上述需求的实现有关，因为无关的改动会增加我review代码的工作量。输出修改后的完整`08_category_operations.php`和`08_web_crossRef_query.php`代码。
...
1. 无论是 title 检索模式还是 doi 检索模式，返回的每条论文检索结果下面都会显示 `复制base32按钮` 和 `标签` 按钮，现在能不能在这两个按钮的右边新增一个 `前往论文` 按钮，要求样式和这两个按钮一致，包括间距、按钮字体大小、样式等统一，区别就是填充的颜色要有区别，但要美观。点击 `前往论文` 按钮时，会在浏览器新的标签页打开相应论文的页面： `https://doi.org/+doi`，从而实现到相应论文的跳转。
...
修改 `08_web_crossRef_query.php` 及其他相关脚本代码，实现以下功能：

1. 无论是 title 检索模式还是 doi 检索模式结果中，在 `复制base32按钮` 、 `标签` 、`前往论文` 、`复制doi`、`预览`等按钮旁边新增一个 `分类`按钮，样式和风格和其他按钮一致。
2. `分类`和`标签`的功能和触发的操作几乎一样，唯一的区别在于点击 `标签` 打开的分类窗口中各个分类标签是按照 `Categories` 表中的默认顺序从上到下、从左到右依次罗列的，顺序是相对固定的。现在我希望`分类`标签按钮打开的分类窗口中各个分类标签是按照一定顺序排列的，这个顺序是基于`Papers`表中的`paperID`来定义的。首先按照`paperID`降序将每篇论文所属的分类标签`category_name`添加到一个列表中，由于每篇论文所属至少一个分类标签，对于一篇论文所属多个标签时，可以按照`categoryID`大的标签放在前面（因为最新创建的分类标签往往是当前更关注的标签）的顺序放入到列表中。除此之外，列表中会有很多重复元素，现在对这个列表中的分类标签进行去重，列表中仅保留每个标签第一次出现时的值，后续重复的值全部删掉。通过这样操作，就能够使得`paperID`越大的论文所使用的分类标签在`分类`打开的窗口中排的就越靠前（因为越新添加的论文往往也是当前越关注的论文，其所属的分类标签可能也是当前使用频率最高的，因此需要放在最前面）。上面的实现方式可能不是最高效的，仅作为参考，你可以采用更高效的算法来实现上述排序目的。
3. 除上面的分类标签顺序之外，`分类`实现的功能和样式应与`标签`按钮一致。
...
请修改 `08_web_crossRef_query.php` 中分类选择弹窗的界面逻辑。

初始版本中，分类选择弹窗只有一个位于弹窗底部的 `保存分类` 按钮。用户如果在弹窗顶部、上部或中部完成分类勾选，需要滚动到底部才能保存，不够方便。

请将分类选择弹窗调整为：只保留一个位于弹窗顶部区域、固定可见的 `保存` 按钮。用户滚动分类列表时，顶部标题栏和 `保存` 按钮应始终可见；底部原有的 `保存分类` 按钮应移除，避免两个保存入口造成功能重复和界面冗余。

要求：

1. 顶部按钮文字使用 `保存`。
2. 移除初始版本中位于底部的 `保存分类` 按钮。
3. 顶部 `保存` 按钮应执行初始版本中底部 `保存分类` 按钮的同一套保存分类逻辑。
4. 不要复制一套独立保存逻辑；应复用或迁移原有保存事件处理器、保存函数或提交逻辑，避免行为不一致。
5. 顶部区域应固定在弹窗顶部，用户滚动分类列表时仍然可见。
6. 分类列表应单独滚动，顶部固定区域不能覆盖分类内容。
7. 顶部固定区域不能影响用户勾选分类、关闭弹窗或使用其他现有功能。
8. 顶部保存按钮视觉上应保持克制，避免过度抢眼；尽量使用与现有页面风格协调的按钮样式。
9. 顶部标题栏上下空间不要过大，应保持紧凑，避免占用过多分类浏览区域。
10. 注意：初始代码中 `标签` 按钮和 `分类` 按钮实际共用同一个分类选择弹窗，因此该修改会同时影响两个入口。修改时应确认两个入口打开弹窗、分类勾选、保存、关闭都正常。
11. 如果初始保存流程包含关闭弹窗、刷新状态、错误提示、后端更新等行为，顶部 `保存` 按钮必须保持一致。
